...
module.exports = ({
    ...
    return new Promise((resolve, reject) => {
        ...
        ws.cell(1, 1, 1, 6, true).string(`Детализация проживания № ${number}`).style(titleStyle);
        ws.cell(2, 1, 2, 6, true).string(`за ${period}`).style(subtitleStyle);

        ws.cell(3, 1, 3, 2, true).string('Срок проживания');
        ws.cell(3, 3).string('Адрес');
        ws.cell(3, 4).string('Клиент');
        ws.cell(3, 5).string('Кол-во дней');
        ...
        data.forEach(item => {
            ...
            ws.cell(currentRow, 5).number(+item[4]);
            ws.cell(currentRow, 6).number(+item[5]);
        ...
        ws.cell(currentRow, 1, currentRow, 4, true).string('Итого дней:');
        ws.cell(currentRow, 5, currentRow, 6, true).number(totalDays);
        ...
        const fileName = `detailing-apartments.${new Date().getTime()}.xlsx`;
