...
async function main() {
    ...
    const documentNumber = 228;
    const documentCreated = moment().format('DD.MM.YYYY');
    const takeFrom = 'Юр. лицо';
    const documentSum = 1488.32;
    const sumLeft = documentSum.toString().split('.')[0];
    const sumRight = documentSum.toString().split('.')[1];
    const base = 'Акт выполненных работ № 15 от 07.08.2019';
    ...
    await wb.toFileAsync(path.join(process.cwd(), 'uploads', 'income' + new Date().getTime() + '.xlsx'));
}
